    ...
    if ("batch" %in% names(df_mod)) {
        ...
        if (protadj) {
            ...
            part_mod <- full_mod %>%
                ...
                dplyr::filter(
                    dplyr::n_distinct(is.na(df_res[key_grp == "G0"])) == 1,
                    dplyr::n_distinct(is.na(df_res[key_grp == "G1"])) == 1
                ) %>%
                dplyr::ungroup() %>%
            ...
            diff_mod <- full_mod %>%
                dplyr::group_by(protsite) %>%
                dplyr::filter(all(!is.na(df_res))) %>%
                dplyr::group_by(protsite, batch, df_res) %>%
